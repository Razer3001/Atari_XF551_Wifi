...
// ========= Debug último WRITE visto desde el Atari =========
uint8_t  lastWriteDev        = 0;
uint16_t lastWriteSec        = 0;
uint8_t  lastWriteDD         = 0;
uint16_t lastWriteLen        = 0;
uint8_t  lastWriteHash       = 0;
uint8_t  lastWritePreview[16];
...
void handleSioHeader(uint8_t f[5]){
  ...
  if (base == 0x53){
    ...
    if (n != 4){
      // si no hay SLAVE, devolvemos un STATUS genérico como fallback
      st[0] = 0x00;
      st[1] = 0x80;
      st[2] = 0x01;
      st[3] = 0x00;
      logf("[SIO] STATUS remoto falló %s, usando STATUS genérico", devName(dev));
    }
  ...
  if (base == 0x21){
    ...
    int n = formatRemoteDisk(dev, cmd, sec, dd, fmt);
    if (n <= 0){
      SerialSIO.write(SIO_ERROR);
      SerialSIO.flush();
  ...
  if (base == 0x50 || base == 0x57){
    ...
    // DEBUG: track último WRITE visto desde el Atari
    uint8_t h = 0;
    ...
    lastWriteSec  = sec;
    lastWriteDD   = dd ? 1 : 0;
    lastWriteLen  = expectedLen;
    lastWriteHash = h;
    memcpy(lastWritePreview, buf, 16);
    logf("[DBG] LAST_WRITE %s sec=%u dd=%s len=%d hash=0x%02X",
         devName(dev), sec, dd ? "DD" : "SD", expectedLen, h);
...
void loop(){
  ...
  // timeout de presencia de SLAVES
  unsigned long now = millis();
  for (int i = 0; i < 4; i++){
  ...
  // === Captura de frames SIO desde el Atari (estilo código viejo) ===
  if (digitalRead(SIO_COMMAND) == LOW) {
    // damos tiempo a que el Atari deposite el frame completo
    delayMicroseconds(2500);

    if (SerialSIO.available() >= 5) {
      uint8_t f[5];
      for (int i = 0; i < 5; ++i) {
        f[i] = SerialSIO.read();
      }
    ...
    // limpia cualquier byte residual
