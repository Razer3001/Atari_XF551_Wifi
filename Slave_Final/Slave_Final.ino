 ...
 *   • FORMAT (0x21 / 0x22) → siempre 128 bytes de resultado
 ...
 *   • Para READ/STATUS/FORMAT/PERCOM devuelve TYPE_SECTOR_CHUNK al MASTER
 *   • Para WRITE:
...
// FORMAT genérico (cmd = 0x21 o 0x22) – la XF551 devuelve SIEMPRE 128 bytes
void handleFormatFromMasterCmd(uint8_t dev, uint8_t cmd, uint16_t param) {
  uint8_t aux1 = (uint8_t)(param & 0xFF);
  uint8_t aux2 = (uint8_t)(param >> 8);

  uint8_t frame[5];
  frame[0] = dev;
  frame[1] = cmd;   // 0x21 (INDEX) o 0x22 (MEDIUM/ENHANCED)
  frame[2] = aux1;
  frame[3] = aux2;
  frame[4] = sioChecksum(frame, 4);

  logf("[SLAVE D%u] FORMAT cmd=0x%02X aux1=%u aux2=%u (resultado=128 bytes)",
       (unsigned)(dev - 0x30), cmd, aux1, aux2);

  pulseCommandAndSendFrame(frame);

  if (!waitByte(SIO_ACK, 4000)) {
    logf("[SLAVE D%u] FORMAT 0x%02X: sin ACK del drive",
         (unsigned)(dev - 0x30), cmd);
    sendNAK();
    return;
  }

  const int resultLen = SECTOR_128;
  uint8_t fmt[MAX_SECTOR_BYTES];
  const unsigned long FORMAT_TIMEOUT_MS = 90000;

  logf("[SLAVE D%u] FORMAT 0x%02X: esperando resultado (%d bytes, timeout=%lu ms)",
       (unsigned)(dev - 0x30), cmd, resultLen, FORMAT_TIMEOUT_MS);

  if (!readFromDrive(fmt, resultLen, FORMAT_TIMEOUT_MS)) {
    logf("[SLAVE D%u] FORMAT 0x%02X: fallo lectura resultado",
         (unsigned)(dev - 0x30), cmd);
    sendNAK();
    return;
  }
  ...
  logf("[SLAVE D%u] FORMAT 0x%02X OK (%d bytes resultado)",
       (unsigned)(dev - 0x30), cmd, resultLen);

  cacheCount = 0;
}

// (opcional: versión antigua 0x21, la dejamos por si la quieres usar en algo más)
void handleFormatFromMaster(uint8_t dev, uint16_t param, bool dd) {
  (void)dd;
  handleFormatFromMasterCmd(dev, 0x21, param);
}

void handlePercomReadFromMaster(uint8_t dev){
  uint8_t frame[5];
  frame[0] = dev;
  frame[1] = 0x4E;
  frame[2] = 0x00;
  ...
  logf("[SLAVE D%u] READ PERCOM 0x4E",
       (unsigned)(dev - 0x30));

  pulseCommandAndSendFrame(frame);

  if (!waitByte(SIO_ACK, 3000)){
    logf("[SLAVE D%u] PERCOM 0x4E: sin ACK del drive", (unsigned)(dev - 0x30));
    sendNAK();
    return;
  }

  uint8_t blk[PERCOM_BLOCK_LEN];
  if (!readFromDrive(blk, PERCOM_BLOCK_LEN, 5000)){
    logf("[SLAVE D%u] PERCOM 0x4E: fallo lectura bloque", (unsigned)(dev - 0x30));
    sendNAK();
    return;
  }

  sendACK();
  sendSectorChunk(0, blk, PERCOM_BLOCK_LEN);
  logf("[SLAVE D%u] PERCOM READ OK",
       (unsigned)(dev - 0x30));
}

void handlePercomWriteFromMaster(uint8_t dev){
  if (!percomBufReady || percomLen < PERCOM_BLOCK_LEN){
    logf("[SLAVE D%u] PERCOM WRITE: datos no listos len=%d",
         (unsigned)(dev - 0x30), percomLen);
    sendNAK();
    return;
  }
  ...
  logf("[SLAVE D%u] WRITE PERCOM 0x4F",
       (unsigned)(dev - 0x30));

  pulseCommandAndSendFrame(frame);

  if (!waitByte(SIO_ACK, 3000)){
    logf("[SLAVE D%u] PERCOM 0x4F: sin ACK del drive", (unsigned)(dev - 0x30));
    sendNAK();
    return;
  }
  ...
  memset(tmp, 0, sizeof(tmp));
  int copyLen = (percomLen > PERCOM_BLOCK_LEN) ? PERCOM_BLOCK_LEN : percomLen;
  memcpy(tmp, percomWriteBuf, copyLen);
  ...
  if (!waitByte(SIO_COMPLETE, 5000)){
    logf("[SLAVE D%u] PERCOM 0x4F: sin COMPLETE del drive", (unsigned)(dev - 0x30));
    sendNAK();
  } else {
    sendACK();
    logf("[SLAVE D%u] PERCOM WRITE OK",
         (unsigned)(dev - 0x30));
  }
...
void onDataRecv(const esp_now_recv_info_t *info, const uint8_t* in, int len){
  ...
  if (type == TYPE_SECTOR_CHUNK && len >= 6){
    ...
    if (sec == PERCOM_SEC_MAGIC){
      ...
      int copyLen = payload;
      if (off + copyLen > PERCOM_BLOCK_LEN)
        copyLen = max(0, PERCOM_BLOCK_LEN - off);

      if (copyLen > 0){
        memcpy(percomWriteBuf + off, in + 6, copyLen);
        percomLen += copyLen;
      }

      if (ci == (cc - 1)){
        percomBufReady = true;
        logf("[SLAVE D%u] PERCOM WRITE DATA len=%d chunks=%u",
             (unsigned)(dev - 0x30), percomLen, cc);
      }
      return;
    }
  ...
  if (type == TYPE_CMD_FRAME && len >= 6){
    ...
    if (base == 0x21 || base == 0x22){
      handleFormatFromMasterCmd(dev, cmd, sec);
      return;
    }

    if (base == 0x4E){
      handlePercomReadFromMaster(dev);
      return;
    }

    if (base == 0x4F){
      handlePercomWriteFromMaster(dev);
      return;
    }
